...
print("--- COLOR-FLEX BATCH RENDERER v2.5 (multi-scale + multi-scene) ---")

# Parse command line arguments
# NOTE: For furniture, we use --piece and --piece-object.
# Back-compat: we still accept --garment and --garment-object.
target_collection = "botanicals"  # Default fallback
piece_name = "sofa"               # Default furniture piece subdirectory
piece_object = "sofa"             # Default furniture piece object name in Blender

for arg in sys.argv:
    if arg.startswith("--collection="):
        target_collection = arg.split("=", 1)[1]
    elif arg.startswith("--piece="):
        piece_name = arg.split("=", 1)[1]
    elif arg.startswith("--piece-object="):
        piece_object = arg.split("=", 1)[1]
    # Back-compat with clothing runners
...
print(f"🪑 Piece: {piece_name} (object: {piece_object})")
...
CONFIG: Dict[str, Any] = {
    ...
    # Target collection and piece (from command line)
    "target_collection": target_collection,
    "piece_name": piece_name,

    # Output style (collection suffix)
    "collection_suffix": "fur",            # "clo" (clothing) vs "fur" (furniture)
    ...
    # Piece object and materials (from command line)
    "mockup_object": piece_object,           # the furniture piece OBJECT name
    ...
    # Multi-scale rendering (pattern sizes on furniture)
    # The render_scales are the numeric UV mapping scales used in Blender.
    # The scale_labels are the keys/labels used in your multi-res collections (start at 0.5).
...
def render_pattern(pattern_data):
    ...
    # Output: <output_root>/<collection>-fur/layers/<piece>
    ...
    # Piece object
    piece_obj = bpy.data.objects.get(CONFIG["mockup_object"])
    if not piece_obj:
        print(f"  ❌ Piece object '{CONFIG['mockup_object']}' not found")
        return False
    ...
    for uv_scale, scale_label in zip(render_scales, scale_labels):
        ...
        for i, layer in enumerate(layers[: CONFIG["max_layers"]]):
            ...
            try:
                mat = _ensure_material_on_object(piece_obj, mat_name)
            except Exception as e:
                print(f"    ❌ {e}")
                continue
    ...
    if CONFIG["generate_manifest"] and all_scales_data:
        manifest = {
            ...
            "piece": CONFIG["piece_name"],
            "piece_object": CONFIG["mockup_object"],
            "pattern": name,
            "pattern_slug": slug,
            "collection": CONFIG["target_collection"],
            "tiling_type": tiling_type,
            "style": f"{CONFIG['target_collection']}-furniture",
...
def main():
    start = time.time()
    print(f"Starting batch render for '{CONFIG['target_collection']}' → node-swap pipeline (furniture)")
    print("-" * 60)

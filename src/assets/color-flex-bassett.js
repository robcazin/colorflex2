...
if(typeof window!=='undefined'){if(!window.__colorflexGlobalDiagnostics){window.__colorflexGlobalDiagnostics={startedAt:new Date().toISOString(),events:[]};}if(typeof window.colorflexDumpDiagnostics!=='function'){window.colorflexDumpDiagnostics=function(){var persisted=null;try{var raw=sessionStorage.getItem('colorflexLastDiagnostics');if(raw)persisted=JSON.parse(raw);}catch(_){}return{submit:window.__colorflexLastSubmitDiagnostic||persisted&&persisted.submit||null,proofRender:window.__colorflexLastProofRenderSnapshot||persisted&&persisted.proofRender||null,restore:window.__colorflexLastRestoreSnapshot||persisted&&persisted.restore||null,global:window.__colorflexGlobalDiagnostics||persisted&&persisted.global||null};};}if(!window.__colorflexDiagFetchWrapped&&typeof window.fetch==='function'){window.__colorflexDiagFetchWrapped=true;var __cfOrigFetch=window.fetch.bind(window);window.fetch=/*#__PURE__*/function(){var _ref=_asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee(url,options){var urlString,isCartAdd,body,payload,_iterator,_step,_step$value,k,v,layerInputsSnapshot,currentLayersSnapshot;return _regenerator().w(function(_context){while(1)switch(_context.n){case 0:try{urlString=typeof url==='string'?url:url&&url.url?url.url:'';isCartAdd=/\/cart\/add(\.js)?/i.test(urlString);if(isCartAdd){body=options&&options.body;payload={};if(body instanceof FormData){try{_iterator=_createForOfIteratorHelper(body.entries());try{for(_iterator.s();!(_step=_iterator.n()).done;){_step$value=_slicedToArray(_step.value,2),k=_step$value[0],v=_step$value[1];payload[k]=typeof v==='string'?v:'[binary]';}}catch(err){_iterator.e(err);}finally{_iterator.f();}}catch(e){payload.__formDataReadError=String(e&&e.message?e.message:e);}}else if(typeof body==='string'){payload.__raw=body.length>4000?body.slice(0,4000)+'...[truncated]':body;try{payload.__json=JSON.parse(body);}catch(e){}}else if(body&&_typeof(body)==='object'){payload.__object=body;}layerInputsSnapshot=Array.isArray(appState.layerInputs)?appState.layerInputs.map(function(li,i){return{i:i,label:li&&li.label,value:li&&li.input?li.input.value:null};}):[];currentLayersSnapshot=Array.isArray(appState.currentLayers)?appState.currentLayers.map(function(l,i){return{i:i,label:l&&l.label,color:l&&l.color,isShadow:!!(l&&l.isShadow)};}):[];window.__colorflexLastSubmitDiagnostic={at:new Date().toISOString(),source:'global-fetch-monitor',requestUrl:urlString,payload:payload,appState:{pattern:appState.currentPattern?appState.currentPattern.name:null,collection:appState.selectedCollection?appState.selectedCollection.name:null,layerInputs:layerInputsSnapshot,currentLayers:currentLayersSnapshot},proofRender:window.__colorflexLastProofRenderSnapshot||null,restore:window.__colorflexLastRestoreSnapshot||null};window.__colorflexGlobalDiagnostics.events.push({at:new Date().toISOString(),type:'cart_add_observed',requestUrl:urlString});try{sessionStorage.setItem('colorflexLastDiagnostics',JSON.stringify({submit:window.__colorflexLastSubmitDiagnostic||null,proofRender:window.__colorflexLastProofRenderSnapshot||null,restore:window.__colorflexLastRestoreSnapshot||null,global:window.__colorflexGlobalDiagnostics||null}));}catch(_){}}}catch(diagErr){try{window.__colorflexGlobalDiagnostics.events.push({at:new Date().toISOString(),type:'diag_error',message:String(diagErr&&diagErr.message?diagErr.message:diagErr)});}catch(_){}}return _context.a(2,__cfOrigFetch(url,options));}},_callee);}));return function(_x,_x2){return _ref.apply(this,arguments);};}();}}var BACKGROUND_INDEX=0;var FURNITURE_BASE_INDEX=1;var PATTERN_BASE_INDEX=2;var isAppReady=false;// Flag to track if the app is fully initialized
...
var STANDARD_COLLECTION_NAMES=['abundance','pages','galleria','oceana','ancient-tiles'];// Legacy fallback: patterns that are standard but may still have colorFlex: true in JSON (until data pipeline sets colorFlex: false).
...
var LEGACY_STANDARD_PATTERNS_BY_COLLECTION={'farmhouse':new Set(['Laundry Daze Original Colors','On Antique Letters','Folkart Floral On Black','Sewing Daze Version 2','Sewing Daze Version 1']),'english-cottage':new Set(['Greenwich Time'])};function normalizedCollectionName(col){var _col$name,_col$name$toLowerCase;var name=col===null||col===void 0||(_col$name=col.name)===null||_col$name===void 0||(_col$name$toLowerCase=_col$name.toLowerCase)===null||_col$name$toLowerCase===void 0?void 0:_col$name$toLowerCase.call(_col$name);return name?name.replace(/\s+/g,'-'):'';}function patternNameMatches(pattern,standardName){if(!(pattern!==null&&pattern!==void 0&&pattern.name))return false;if(pattern.name===standardName)return true;var norm=pattern.name.toLowerCase().replace(/[\s-]+/g,'-');var standardNorm=standardName.toLowerCase().replace(/[\s-]+/g,'-');return norm===standardNorm;}/** Returns true if pattern should be treated as standard (no layer UI, thumbnail-only). */function patternIsStandard(pattern,collection){var _col$name2,_col$name2$toLowerCas;if(!pattern)return false;// 1. Universal: explicit pattern flag (source of truth). Set colorFlex: false in JSON for any "Standard Pattern" so new collections need no code change.
...
colorLayersForSave=state.currentLayers.filter(function(layer){return layer&&!layer.isShadow&&layer.color!=null&&String(layer.color).trim()!=='';});// Capture current pattern state - match Liquid template structure
currentState={collectionName:((_state$selectedCollec2=state.selectedCollection)===null||_state$selectedCollec2===void 0?void 0:_state$selectedCollec2.name)||'Unknown',patternName:state.currentPattern.name,colors:colorLayersForSave.map(function(layer){return{label:layer.label,color:layer.color,// Store original SW-formatted color for pattern ID generation
...
var viewSavedButton=document.createElement('div');viewSavedButton.id='viewSavedBtn';viewSavedButton.setAttribute('aria-label','My Designs');viewSavedButton.setAttribute('title','Browse your saved ColorFlex Designs');viewSavedButton.setAttribute('data-tooltip','My Designs — view, edit, and reopen your saved patterns');// Get pattern count for badge
var savedPatterns=JSON.parse(localStorage.getItem('colorflexSavedPatterns')||'[]');viewSavedButton.style.cssText="\n        position: relative;\n        display: inline-flex;\n        align-items: center;\n        justify-content: center;\n        cursor: pointer;\n        padding: 2px;\n        border-radius: 50%;\n        background: rgba(26, 32, 44, 0.9);\n        border: 2px solid rgb(212, 175, 55);\n        transition: all 0.3s ease;\n        z-index: 150;\n        pointer-events: auto;\n        width: 48px;\n        height: 48px;\n        overflow: visible;\n    ";viewSavedButton.innerHTML="\n        <img src=\"".concat(normalizePath('img/camelion-sm-black.jpg'),"\" style=\"width: 100%; height: 100%; border-radius: 50%;\">\n        <span style=\"\n            background: #d4af37;\n            color: #1a202c;\n            font-size: 10px;\n            border-radius: 10px;\n            padding: 2px 4px;\n            position: absolute;\n            top: -8px;\n            right: -8px;\n            min-width: 24px;\n            text-align: center;\n            font-weight: bold;\n            line-height: 1;\n        \">").concat(savedPatterns.length,"</span>\n    ");// Add hover effects
...
).then(/*#__PURE__*/function(){var _ref3=_asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee3(proofCanvas){var infoStripHeight,finalCanvas,finalCtx,baseFontSize,smallFontSize,leftMargin,yPosition,scaleDisplay,filename;return _regenerator().w(function(_context3){while(1)switch(_context3.n){case 0:console.log('✅ Pattern proof generation complete, adding info strip...');// Create a new canvas with extra height for info strip
 ...
 */function deleteSavedPattern(patternRef){try{// Delete from localStorage
var patterns=JSON.parse(localStorage.getItem('colorflexSavedPatterns')||'[]');var updatedPatterns=patterns.slice();var deleteIndex=-1;if(patternRef&&_typeof(patternRef)==='object'){// CFM-SEARCH: DELETE_BY_SAVED_ENTRY_ID
if(patternRef.savedEntryId){deleteIndex=updatedPatterns.findIndex(function(p){return p&&p.savedEntryId&&p.savedEntryId===patternRef.savedEntryId;});}// Legacy fallback for older entries with no savedEntryId.
if(deleteIndex<0){deleteIndex=updatedPatterns.findIndex(function(p){if(!p)return false;if(p.id!==patternRef.id)return false;if(patternRef.timestamp&&p.timestamp===patternRef.timestamp)return true;if(patternRef.thumbnail&&p.thumbnail===patternRef.thumbnail)return true;if(patternRef.colors&&p.colors&&JSON.stringify(p.colors)===JSON.stringify(patternRef.colors))return true;return false;});}// Last resort: remove only one matching id (not all).
if(deleteIndex<0&&patternRef.id){deleteIndex=updatedPatterns.findIndex(function(p){return p&&p.id===patternRef.id;});}}else{// Backward compatibility when called with a string ID.
var patternId=String(patternRef||'');deleteIndex=updatedPatterns.findIndex(function(p){return p&&p.id===patternId;});}if(deleteIndex>=0){updatedPatterns.splice(deleteIndex,1);}localStorage.setItem('colorflexSavedPatterns',JSON.stringify(updatedPatterns));console.log('✅ Pattern deleted from localStorage');// Update menu icon - call both systems for comprehensive coverage
updateSavedPatternsMenuIcon();// 🆕 CHAMELEON BUTTON: Also call global updateMenuIcon if available (from colorflex-menu-icon.js)
...
var scaleDisplay=document.getElementById('furnitureScaleDisplay');if(scaleDisplay){scaleDisplay.textContent="".concat(preservedFurnitureScale,"X");}}// Load the first pattern in the collection (slug preferred for ColorFlex/Bassett)
var firstPattern=targetCollection.patterns[0];if(firstPattern){var firstPatternId=firstPattern.slug||firstPattern.id||firstPattern.name;loadPatternData(targetCollection,firstPatternId);}console.log("\u2705 Switched to collection: ".concat(collectionName));};// Garment selector function
 ...
 */function createCompressedThumbnail(_x5){return _createCompressedThumbnail.apply(this,arguments);}/**
...
var colorsToApply=pattern.colors||(pattern.customColors?pattern.customColors.map(function(c){return{color:c};}):[]);if(colorsToApply&&colorsToApply.length>0){setTimeout(function(){colorsToApply.forEach(function(savedColor,index){if(appState.currentLayers[index]){var colorValue=savedColor.color||savedColor;appState.currentLayers[index].color=colorValue;var colorHex=lookupColor(colorValue);var layerLabel=appState.currentLayers[index].label;var layerInput=appState.layerInputs.find(function(li){return li.label===layerLabel;});if(layerInput){if(layerInput.input)layerInput.input.value=getCleanColorName(colorValue);if(layerInput.circle)layerInput.circle.style.backgroundColor=colorHex;}// Fallback to DOM selector approach
...
var circleSelectors=["#layer-".concat(index," ~ .layer-circle"),"[data-layer-id=\"layer-".concat(index,"\"] .layer-circle"),".layer-circle[data-layer=\"".concat(index,"\"]")];for(var _i=0,_circleSelectors=circleSelectors;_i<_circleSelectors.length;_i++){var selector=_circleSelectors[_i];var circle=document.querySelector(selector);if(circle){circle.style.backgroundColor=colorHex;break;}}}});// 🆕 RESTORE SCALING: Apply saved scale settings if available
...
base=base.replace(/\.[a-z0-9]+$/i,'');// common legacy suffixes: -W60H40, _W60H40, -1, _1, etc.
base=base.replace(/[-_]?w\d+h\d+$/i,'').replace(/[-_]?(\d+)$/i,'');return base.toLowerCase().replace(/[^a-z0-9]+/g,'-')// normalize separators
.replace(/-+/g,'-').replace(/^-|-$/g,'');}/**
 ...
 */function inferMockupIdFromLegacyCollectionMockupPath(collectionMockupPath,mockupsMap){if(!collectionMockupPath||!mockupsMap||_typeof(mockupsMap)!=='object')return null;var slug=_mockupSlugFromPath(collectionMockupPath);if(!slug)return null;// 1) Exact match against mockup ids (canonicalized)
var direct=resolveMockupFromMap(mockupsMap,slug);if(direct&&direct.id)return direct.id;// 2) Match against mockup image basename slugs
var found=Object.values(mockupsMap).find(function(m){var img=mockupEntryImagePath(m);if(!img)return false;var imgSlug=_mockupSlugFromPath(img);if(!imgSlug)return false;return imgSlug===slug||imgSlug.indexOf(slug)!==-1||slug.indexOf(imgSlug)!==-1;});return found&&found.id?found.id:null;}function isValidMockupsPayload(d){return!!(d&&d.mockups&&_typeof(d.mockups)==='object'&&Object.keys(d.mockups).length>0);}function ensureMinimalColorFlexMockups(){if(!window.ColorFlexMockups||_typeof(window.ColorFlexMockups)!=='object'){window.ColorFlexMockups={};}if(!window.ColorFlexMockups['white-dresser']){window.ColorFlexMockups['white-dresser']=Object.assign({},WHITE_DRESSER_FALLBACK_MAP_ENTRY);}}/** When mockups.json is missing or merge skipped, collections that only have mockupId still need a `mockup` path. */function applyWhiteDresserMockupToCollectionsMissingPath(collections){var _fb$shadow,_fb$shadow2;if(!Array.isArray(collections))return;ensureMinimalColorFlexMockups();var fb=window.ColorFlexMockups['white-dresser'];var img=mockupEntryImagePath(fb);var sh=typeof fb.shadow==='string'?fb.shadow:((_fb$shadow=fb.shadow)===null||_fb$shadow===void 0?void 0:_fb$shadow.path)||((_fb$shadow2=fb.shadow)===null||_fb$shadow2===void 0?void 0:_fb$shadow2.url)||'';if(!img)return;collections.forEach(function(collection){if(!collection||_typeof(collection)!=='object')return;var existing=collection.mockup&&String(collection.mockup).trim();if(existing)return;collection.mockup=typeof fb.image==='string'?fb.image:img;if(sh)collection.mockupShadow=sh;if(typeof fb.widthInches==='number')collection.mockupWidthInches=fb.widthInches;if(typeof fb.heightInches==='number')collection.mockupHeightInches=fb.heightInches;console.warn('⚠️ Collection "'+(collection.name||'')+'" had no resolved mockup path; applied white-dresser fallback.');});}/**
 ...
 */function getEffectiveRoomMockup(collection){var _collection$mockupWid,_collection$mockupHei;if(!collection)return{mockup:'',mockupShadow:'',mockupWidthInches:90,mockupHeightInches:60,tintMask:''};var id=appState.selectedMockupId;var mockups=window.ColorFlexMockups||{};var m=id?resolveMockupFromMap(mockups,id):null;if(m){var _m$image,_m$image2,_m$shadow,_m$shadow2,_m$tintMask,_m$tintMask2,_m$widthInches,_m$heightInches;var image=typeof m.image==='string'?m.image:((_m$image=m.image)===null||_m$image===void 0?void 0:_m$image.path)||((_m$image2=m.image)===null||_m$image2===void 0?void 0:_m$image2.url)||m.path||'';var shadow=typeof m.shadow==='string'?m.shadow:((_m$shadow=m.shadow)===null||_m$shadow===void 0?void 0:_m$shadow.path)||((_m$shadow2=m.shadow)===null||_m$shadow2===void 0?void 0:_m$shadow2.url)||'';var tintMask=typeof m.tintMask==='string'?m.tintMask:((_m$tintMask=m.tintMask)===null||_m$tintMask===void 0?void 0:_m$tintMask.path)||((_m$tintMask2=m.tintMask)===null||_m$tintMask2===void 0?void 0:_m$tintMask2.url)||'';return{mockup:image,mockupShadow:shadow,mockupWidthInches:(_m$widthInches=m.widthInches)!==null&&_m$widthInches!==void 0?_m$widthInches:90,mockupHeightInches:(_m$heightInches=m.heightInches)!==null&&_m$heightInches!==void 0?_m$heightInches:60,tintMask:tintMask||''};}return{mockup:collection.mockup||'',mockupShadow:collection.mockupShadow||'',mockupWidthInches:(_collection$mockupWid=collection.mockupWidthInches)!==null&&_collection$mockupWid!==void 0?_collection$mockupWid:90,mockupHeightInches:(_collection$mockupHei=collection.mockupHeightInches)!==null&&_collection$mockupHei!==void 0?_collection$mockupHei:60,tintMask:collection.tintMask||''};}/**
 ...
 */function getCollectionDefaultMockupLabel(collection,mockups){if(!collection||!mockups)return'Collection Default';var map=mockups||{};var fromId=resolveMockupFromMap(map,collection.mockupId);if(fromId&&fromId.name)return fromId.name;var collMockup=normalizePath(collection.mockup||'');if(collMockup){var byImage=Object.values(map).find(function(m){var _m$image3,_m$image4;var p=normalizePath(typeof(m===null||m===void 0?void 0:m.image)==='string'?m.image:(m===null||m===void 0||(_m$image3=m.image)===null||_m$image3===void 0?void 0:_m$image3.path)||(m===null||m===void 0||(_m$image4=m.image)===null||_m$image4===void 0?void 0:_m$image4.url)||(m===null||m===void 0?void 0:m.path)||'');return p&&p===collMockup;});if(byImage&&byImage.name)return byImage.name;}return'Collection Default';}function closeRoomMockupCustomMenu(){if(window._roomMockupDdDocClose){document.removeEventListener('click',window._roomMockupDdDocClose,true);window._roomMockupDdDocClose=null;}if(window._roomMockupDdEscape){document.removeEventListener('keydown',window._roomMockupDdEscape,true);window._roomMockupDdEscape=null;}var menu=document.getElementById('roomMockupDropdownMenu');var trig=document.getElementById('roomMockupDropdownTrigger');if(menu)menu.hidden=true;if(trig)trig.setAttribute('aria-expanded','false');}function openRoomMockupCustomMenu(){var menu=document.getElementById('roomMockupDropdownMenu');var trig=document.getElementById('roomMockupDropdownTrigger');if(!menu||!trig)return;menu.hidden=false;trig.setAttribute('aria-expanded','true');var closer=function closer(e){var wrap=document.getElementById('roomMockupDropdown');if(wrap&&!wrap.contains(e.target))closeRoomMockupCustomMenu();};window._roomMockupDdDocClose=closer;window._roomMockupDdEscape=function(e){if(e.key==='Escape')closeRoomMockupCustomMenu();};setTimeout(function(){document.addEventListener('click',closer,true);document.addEventListener('keydown',window._roomMockupDdEscape,true);},0);}/**
 * Width for custom mockup picker: fit longest label, capped by mockup box (right-aligned, grows left).
 */function setRoomMockupCustomDropdownWidth(wrap,labels){if(!wrap||!dom.roomMockup)return;var ctx=document.createElement('canvas').getContext('2d');ctx.font='17px "Special Elite", monospace';var max=0;labels.forEach(function(t){var w=ctx.measureText(t).width;if(w>max)max=w;});var pad=52;var roomW=dom.roomMockup.clientWidth||dom.roomMockup.offsetWidth||700;var cap=Math.max(100,roomW-24);var width=Math.min(Math.ceil(max+pad),cap);wrap.style.width="".concat(Math.max(100,Math.floor(width*0.85)),"px");}/**
...
resolved=base+'/'+encodedPath;var isLayer=path.indexOf('layers/')!==-1;var isThumb=path.indexOf('thumbnails/')!==-1;if(window.COLORFLEX_DEBUG_URLS||(isLayer||isThumb)&&_colorFlexLayerUrlLogCount+_colorFlexThumbUrlLogCount<_COLORFLEX_MAX_RESOLVED_LOGS){console.log('[ColorFlex] resolved:',path,'→',resolved);if(isLayer)_colorFlexLayerUrlLogCount++;else if(isThumb)_colorFlexThumbUrlLogCount++;}return resolved;}// Store furniture view settings globally for consistency
...
var currentCollection=(_appState$selectedCol0=appState.selectedCollection)===null||_appState$selectedCol0===void 0?void 0:_appState$selectedCol0.name;if(!currentCollection){console.log("\u274C No collection currently selected");return{error:"No collection selected"};}console.log("\u2705 Current collection: ".concat(currentCollection));// Check compatibility
...
function addTryFurnitureButton(){addTryFurnitureButtonDebounced();}function addTryFurnitureButtonInternal(){var _appState$selectedCol17;// Performance optimization - avoid excessive logging unless in debug mode
...
var currentCollection=(_appState$selectedCol17=appState.selectedCollection)===null||_appState$selectedCol17===void 0?void 0:_appState$selectedCol17.name;if(!currentCollection){if(window.COLORFLEX_DEBUG){console.log("No current collection, skipping furniture button");}return;}// Skip for clothing collections - they auto-show mockup without button
...
function addBackToPatternsButton(){var _appState$selectedCol18,_appState$selectedCol19;console.log("🔙 addBackToPatternsButton() called");// Don't show "Back to Patterns" button in clothing mode (it's only for furniture mode)
var isClothingMode=((_appState$selectedCol18=appState.selectedCollection)===null||_appState$selectedCol18===void 0||(_appState$selectedCol18=_appState$selectedCol18.name)===null||_appState$selectedCol18===void 0?void 0:_appState$selectedCol18.includes('-clo'))||((_appState$selectedCol19=appState.selectedCollection)===null||_appState$selectedCol19===void 0||(_appState$selectedCol19=_appState$selectedCol19.name)===null||_appState$selectedCol19===void 0?void 0:_appState$selectedCol19.includes('.clo-'));if(isClothingMode){console.log("👗 Skipping Back to Patterns button in clothing mode");return;}var existingButton=document.getElementById('backToPatternsBtn');if(existingButton){console.log("🗑️ Removing existing back button");existingButton.remove();}var button=document.createElement('button');button.id='backToPatternsBtn';button.innerHTML="\n        <span>\u2190 Back to Patterns</span>\n    ";button.style.cssText="\n        position: absolute;\n        bottom: 10px;\n        left: 10px;\n        background: linear-gradient(135deg, #ff7b7b 0%, #667eea 100%);\n        color: white;\n        border: none;\n        padding: 12px 18px;\n        border-radius: 25px;\n        font-family: 'Special Elite', monospace;\n        font-size: 14px;\n        font-weight: bold;\n        cursor: pointer;\n        box-shadow: 0 4px 15px rgba(0,0,0,0.2);\n        transition: all 0.3s ease;\n        z-index: 100;\n    ";console.log("🔗 Adding click event listener to back button");button.addEventListener('click',function(event){console.log("🔙 Back button clicked!");event.stopPropagation();// Prevent zoom handler from receiving this event
...
if(!collectionName||collectionName==="UNKNOWN"||collectionName===patternName){var _appState$selectedCol20;console.error("\u274C Invalid collection name: \"".concat(collectionName,"\""));console.error("   Pattern name: \"".concat(patternName,"\""));console.error("   These should be different!");// Try to get it from the current furniture collection
var fallbackCollectionName=(_appState$selectedCol20=appState.selectedCollection)===null||_appState$selectedCol20===void 0?void 0:_appState$selectedCol20.originalCollectionName;if(fallbackCollectionName){console.log("\uD83D\uDD27 Using fallback collection name: \"".concat(fallbackCollectionName,"\""));collectionName=fallbackCollectionName;}else{console.error("\u274C No fallback collection name available!");return[];}}var patternSlug=createPatternSlug(patternName);// Strip -fur suffix from collection name for furniture directory paths
...
showCompatibility:function showCompatibility(){console.log("\uD83D\uDD17 FURNITURE COMPATIBILITY:");Object.entries(furnitureConfig||{}).forEach(function(_ref1){var _ref10=_slicedToArray(_ref1,2),furnitureId=_ref10[0],furniture=_ref10[1];console.log("".concat(furniture.name,": ").concat(furniture.compatibleCollections.join(', ')));});},// Generate folder creation script
...
function checkFurnitureImplementationStatus(){var _appState$selectedCol21,_appState$collections7;// Skip in clothing mode
...
var currentCollection=(_appState$selectedCol21=appState.selectedCollection)===null||_appState$selectedCol21===void 0?void 0:_appState$selectedCol21.name;if(!currentCollection){console.log("\u274C No collection currently selected");return;}console.log("\u2705 Current collection: ".concat(currentCollection));// Check compatibility
var compatible=getCompatibleFurniture(currentCollection);console.log("\u2705 Compatible furniture: ".concat(compatible.length," pieces"));compatible.forEach(function(f){return console.log("   - ".concat(f.name));});// Check if Try Furniture button should be visible
...
if(!appState.currentPattern){_context23.n=3;break;}console.log('🔄 Re-rendering with new furniture...');_context23.n=3;return updatePreview();case 3:return _context23.a(2);}},_callee17);}));return _switchFurniture.apply(this,arguments);}window.switchFurniture=switchFurniture;dom._patternName=document.getElementById("patternName");// Initial assignment
...
var swMatch=colorName.match(/^(SW|SC)\s*(\d+)\s*(.+)$/i);if(swMatch){var prefix=swMatch[1].toUpperCase();var number=swMatch[2];var name=swMatch[3].trim();return"".concat(prefix).concat(number," ").concat(toColorInitialCaps(name));}// If no SW number, try to look it up in colorsData
if(appState&&appState.colorsData){var cleanName=colorName.toLowerCase().trim();var colorEntry=appState.colorsData.find(function(c){return c.color_name&&c.color_name.toLowerCase().trim()===cleanName||c.name&&c.name.toLowerCase().trim()===cleanName;});if(colorEntry&&colorEntry.sw_number){return"".concat(colorEntry.sw_number.toUpperCase()," ").concat(toColorInitialCaps(colorEntry.color_name||colorEntry.name));}}// Fallback: just format the name consistently
return toColorInitialCaps(colorName);}// Lookup color from colors.json data
...
console.log('🦎 Adding chameleon menu icon on page:',window.location.pathname);var patterns=JSON.parse(localStorage.getItem('colorflexSavedPatterns')||'[]');var existingIcon=document.getElementById('colorflexMenuIcon');var isColorFlexPage=/\/pages\/colorflex(?:-|$)/i.test(window.location.pathname||'');if(patterns.length>0&&!existingIcon){var menuIcon=document.createElement('div');menuIcon.id='colorflexMenuIcon';menuIcon.style.cssText="\n            position: relative;\n            display: inline-flex;\n            align-items: center;\n            justify-content: center;\n            cursor: pointer;\n            margin: 0 10px;\n            padding: 8px;\n            border-radius: 50%;\n            background: rgba(212, 175, 55, 0.1);\n            border: 2px solid rgba(212, 175, 55, 0.6);\n            transition: all 0.3s ease;\n            z-index: 1000;\n            width: 44px;\n            height: 44px;\n            box-sizing: border-box;\n            overflow: visible;\n        ";menuIcon.innerHTML="\n            <img src=\"".concat(normalizePath('img/camelion-sm-black.jpg'),"\" \n                 style=\"width: 100%; height: 100%; border-radius: 50%;\" \n                 alt=\"My ColorFlex Patterns\">\n            <span style=\"\n                position: absolute;\n                top: -6px;\n                right: -6px;\n                background: #d4af37;\n                color: white;\n                border-radius: 50%;\n                min-width: 18px;\n                height: 18px;\n                padding: 0 4px;\n                font-size: 10px;\n                font-weight: bold;\n                display: flex;\n                align-items: center;\n                justify-content: center;\n            \">").concat(patterns.length,"</span>\n        ");menuIcon.addEventListener('click',function(){console.log('🎨 Opening saved patterns from menu icon');showSavedPatternsModal();});menuIcon.addEventListener('mouseenter',function(){menuIcon.style.background='rgba(212, 175, 55, 0.2)';menuIcon.style.transform='scale(1.1)';});menuIcon.addEventListener('mouseleave',function(){menuIcon.style.background='rgba(212, 175, 55, 0.1)';menuIcon.style.transform='scale(1)';});if(isColorFlexPage){// Keep this icon out of nav flow on ColorFlex pages to prevent layout shifts.
menuIcon.style.position='fixed';menuIcon.style.top='16px';menuIcon.style.right='16px';menuIcon.style.margin='0';menuIcon.style.zIndex='12000';document.body.appendChild(menuIcon);console.log('✅ Added fixed ColorFlex menu icon with',patterns.length,'saved patterns');return;}// Find the main navigation or header to add our icon to
var nav=document.querySelector('nav, header, .header, .navigation, .main-header, .site-header');if(!nav)return;// Try to place it near existing user/account icons
...
isSimpleModeRender=window.COLORFLEX_SIMPLE_MODE===true;activeZoomState=zoomState||{scale:isSimpleModeRender?1.0:furnitureViewSettings.scale,offsetX:isSimpleModeRender?0:furnitureViewSettings.offsetX,offsetY:isSimpleModeRender?0:furnitureViewSettings.offsetY,isZoomed:false};scale=activeZoomState.scale,offsetX=activeZoomState.offsetX,offsetY=activeZoomState.offsetY;console.log("\uD83D\uDD0D drawFurnitureLayer DEBUG for: ".concat(imagePath.split('/').pop()));console.log("   \uD83D\uDCCA ZOOM STATE: scale=".concat(scale,", offset=(").concat(offsetX.toFixed(1),", ").concat(offsetY.toFixed(1),")"));console.log("   \uD83D\uDD12 Using ".concat(zoomState?'PASSED':'GLOBAL'," zoom state"));console.log("   Canvas size: ".concat(width,"x").concat(height));_context28.p=1;console.log("   \uD83D\uDD0D Attempting to load image: ".concat(imagePath));_context28.n=2;return loadImage(imagePath);case 2:img=_context28.v;if(img){_context28.n=3;break;}console.error("❌ Failed to load image:",imagePath);console.error("❌ Image object is null/undefined - check path and CORS");return _context28.a(2);case 3:console.log("   \u2705 Image loaded successfully: ".concat(img.naturalWidth,"x").concat(img.naturalHeight));console.log("   \uD83D\uDCCD Image source: ".concat(img.src));if(highRes)console.log("   \uD83D\uDD0D High-res rendering: ".concat(renderWidth,"x").concat(renderHeight));// ✅ CRITICAL: Check if image actually loaded (not broken)
if(!(img.naturalWidth===0||img.naturalHeight===0)){_context28.n=4;break;}console.error("❌ Image has zero dimensions - image failed to load properly!");console.error("❌ This will cause a solid rectangle instead of pattern!");return _context28.a(2);case 4:// ✅ REVERT: Use original scaling logic (was correct before)
...
tempCtx.imageSmoothingEnabled=true;tempCtx.imageSmoothingQuality="high";if(!(isMask&&tintColor)){_context28.n=8;break;}// ✅ CORRECTED WALL MASK LOGIC
...
tempCtx.drawImage(maskProcessCanvas,drawX,drawY);console.log("   \u2705 Wall mask applied: white areas colored, black areas transparent");_context28.n=23;break;case 8:if(!tintColor){_context28.n=22;break;}// ✅ Use luminance-based logic for furniture bases and pattern layers
...
tempCtx.drawImage(processCanvas,drawX,drawY);_context28.n=21;break;case 12:if(!isExtrasLayer){_context28.n=20;break;}// ✅ EXTRAS LAYER: Tint ONLY within existing alpha channel
...
_i12=0;case 16:if(!(_i12<_data.length)){_context28.n=19;break;}originalAlpha=_data[_i12+3];// Skip fully transparent pixels
if(!(originalAlpha===0)){_context28.n=17;break;}return _context28.a(3,18);case 17:_r2=_data[_i12];_g2=_data[_i12+1];_b2=_data[_i12+2];// Calculate brightness/luminance of original pixel
...
case 18:_i12+=4;_context28.n=16;break;case 19:// Put processed data back to processing canvas
_processCtx.putImageData(_imageData2,0,0);// Now composite the processed image at the correct position on temp canvas
tempCtx.drawImage(_processCanvas,drawX,drawY);console.log("✅ Extras tinting applied (alpha-preserved) with color:",tintColor);_context28.n=21;break;case 20:// Keep original alpha-based logic for other elements (if any)
...
colorCircle.addEventListener('dragover',function(e){e.preventDefault();e.dataTransfer.dropEffect='copy';colorCircle.classList.add('cf-drop-target');});colorCircle.addEventListener('dragleave',function(){colorCircle.classList.remove('cf-drop-target');});colorCircle.addEventListener('drop',function(e){e.preventDefault();colorCircle.classList.remove('cf-drop-target');var payload=parseCuratedDragPayload(e.dataTransfer);if(!payload)return;var displayName=payload.color_name||getCleanColorName(payload.display||'');var stored=payload.display||displayName;var hex=payload.hex?"#".concat(payload.hex.replace(/^#/,'')):lookupColor(stored);if(!hex||hex==='#FFFFFF')return;// Apply to this layer UI
...
var hex=lookupColor(expandedStoredValue||userInput);if(!userInput||hex==="#FFFFFF"){// Invalid input - restore to initial color
...
if(c.color_name&&c.color_name.toLowerCase()===colorNamePart){return true;}return false;});if(!found){console.error("\u274C No color found for curated label: \"".concat(label,"\""));return;}if(!found.hex){console.error("\u274C Missing hex for found color:",found);return;}var hex="#".concat(found.hex);console.log("\u2705 Curated color found: \"".concat(label,"\" -> ").concat(found.sw_number," ").concat(found.color_name," -> ").concat(hex));var circle=document.createElement("div");circle.className="curated-color-circle cursor-pointer";circle.style.backgroundColor=hex;circle.style.setProperty('background-color',hex,'important');var text=document.createElement("span");text.className="text-xs font-bold text-center ".concat(getContrastClass(hex));text.style.cssText='white-space: pre-line; display: block;';text.innerHTML="".concat((_found$sw_number=found.sw_number)===null||_found$sw_number===void 0?void 0:_found$sw_number.toUpperCase(),"<br>").concat(toColorInitialCaps(found.color_name));circle.appendChild(text);// Drag payload so a curated color can be dropped onto a layer circle.
circle.draggable=true;circle.addEventListener('dragstart',function(e){var payload=buildCuratedDragPayload(found);if(!payload)return;try{e.dataTransfer.setData('application/colorflex-color',JSON.stringify(payload));}catch(_unused2){}e.dataTransfer.setData('text/plain',payload.display||label);e.dataTransfer.effectAllowed='copy';});circle.addEventListener("click",function(){var selectedLayer=appState.lastSelectedLayer;if(!selectedLayer)return alert("Please select a layer first.");selectedLayer.input.value=getCleanColorName(found.color_name);selectedLayer.circle.style.backgroundColor=hex;var i=appState.currentLayers.findIndex(function(l){return l.label===selectedLayer.label;});if(i!==-1)appState.currentLayers[i].color=found.color_name;var j=appState.layerInputs.findIndex(function(li){return li.label===selectedLayer.label;});if(j!==-1){appState.layerInputs[j].input.value=getCleanColorName(found.color_name);appState.layerInputs[j].circle.style.backgroundColor=hex;}appState.lastSelectedColor={name:found.color_name,hex:hex};updateDisplays();});container.appendChild(circle);});console.log("✅ Curated colors populated:",colors.length);}function getLayerMappingForPreview(isFurnitureCollection){if(isFurnitureCollection){return{type:'furniture',patternStartIndex:2,// Pattern layers start at index 2  
...
console.log("\uD83D\uDD0D PRE-FILTER: isClothingMode=".concat(isClothingMode,", isFurnitureMode=").concat(isFurnitureMode,", collections=").concat(appState.collections.length));// BASSETT: Exclude standard patterns/collections to avoid CORS on B2 thumbnails; show only ColorFlex.
if(window.COLORFLEX_MODE==='BASSETT'){beforeBassett=appState.collections.length;appState.collections=appState.collections.filter(function(c){var colorFlexPatterns=(c.patterns||[]).filter(function(p){return p.colorFlex===true&&p.layers&&p.layers.length>0;});if(colorFlexPatterns.length===0){console.log("🚫 BASSETT: Excluding collection (no ColorFlex patterns):",c.name);return false;}c.patterns=colorFlexPatterns;return true;});console.log("🎨 BASSETT: Only ColorFlex collections shown:",appState.collections.length,"(excluded",beforeBassett-appState.collections.length,"standard-only)");}if(isClothingMode){// ✅ CRITICAL: Filter to ONLY ColorFlex collections (must have colorFlex: true)
...
if(!(isFurnitureMode||window.COLORFLEX_MODE==='FURNITURE'||window.location.pathname.includes('furniture'))){_context32.n=22;break;}// ✅ CRITICAL: Filter to ONLY ColorFlex collections
...
_context32.n=21;return loadFurnitureConfig();case 21:_context32.n=23;break;case 22:// WALLPAPER MODE (default): Filter out clothing/furniture collections
...
modeDefaultCollection=null;if(window.COLORFLEX_MODE==='BASSETT'){modeDefaultCollection=window.BASSETT_DEFAULT_COLLECTION||'hip-to-be-square';}else if(window.COLORFLEX_MODE==='CLOTHING'){modeDefaultCollection=window.CLOTHING_DEFAULT_COLLECTION||'bombay-clo';}else if(window.COLORFLEX_MODE==='FURNITURE'){modeDefaultCollection=window.FURNITURE_DEFAULT_COLLECTION||'botanicals-fur';}collectionName=((_window$appState5=window.appState)===null||_window$appState5===void 0||(_window$appState5=_window$appState5.selectedCollection)===null||_window$appState5===void 0?void 0:_window$appState5.name)||urlCollectionName||autoLoadCollectionName||modeDefaultCollection;// BASSETT: prefer hip-to-be-square so we never default to another collection
if(window.COLORFLEX_MODE==='BASSETT'&&(!collectionName||collectionName===modeDefaultCollection)){collectionName=window.BASSETT_DEFAULT_COLLECTION||'hip-to-be-square';}console.log("🔍 COLLECTION MATCHING DEBUG:");console.log("  Requested collection name:",collectionName);console.log("  Available collection names:",appState.collections.map(function(c){return c.name;}));selectedCollection=appState.collections.find(function(c){return c&&typeof c.name==='string'&&collectionName&&typeof collectionName==='string'&&c.name.trim().toLowerCase()===collectionName.toLowerCase();});// BASSETT: try normalized name (hyphen/space) so "hip to be square" or "hip-to-be-square" both match
if(!selectedCollection&&window.COLORFLEX_MODE==='BASSETT'&&collectionName&&collectionName.toLowerCase().replace(/\s+/g,'-').indexOf('hip')!==-1){want=(window.BASSETT_DEFAULT_COLLECTION||'hip-to-be-square').toLowerCase().replace(/\s+/g,'-');selectedCollection=appState.collections.find(function(c){if(!c||!c.name)return false;var n=c.name.trim().toLowerCase().replace(/\s+/g,'-');return n===want||n.replace(/-/g,' ')===want.replace(/-/g,' ');});if(selectedCollection)console.log("  ✅ BASSETT: matched default collection by normalized name:",selectedCollection.name);}// ✅ If not found and collectionName has furniture/clothing variant suffix, try matching base name
...
firstColorFlexCollection=appState.collections.find(function(c){var _c$patterns11;return(_c$patterns11=c.patterns)===null||_c$patterns11===void 0?void 0:_c$patterns11.some(function(p){return p.colorFlex===true;});});if(firstColorFlexCollection){selectedCollection=firstColorFlexCollection;console.log("  Using first ColorFlex collection as fallback: ".concat(firstColorFlexCollection.name));}else{console.error("  \u274C No ColorFlex collections found! Using first collection: ".concat((_appState$collections9=appState.collections[0])===null||_appState$collections9===void 0?void 0:_appState$collections9.name));selectedCollection=appState.collections[0];}}else if(window.COLORFLEX_MODE==='BASSETT'){// BASSETT: prefer hip-to-be-square when lookup failed
bassettDefault=(window.BASSETT_DEFAULT_COLLECTION||'hip-to-be-square').toLowerCase().replace(/\s+/g,'-');selectedCollection=appState.collections.find(function(c){if(!c||!c.name)return false;var n=c.name.trim().toLowerCase().replace(/\s+/g,'-');return n===bassettDefault||n.replace(/-/g,' ')===bassettDefault.replace(/-/g,' ');})||appState.collections[0];console.log("  BASSETT fallback:",((_selectedCollection=selectedCollection)===null||_selectedCollection===void 0?void 0:_selectedCollection.name)||"none");}else{selectedCollection=appState.collections[0];console.log("  Using first collection as fallback (no collection specified)");}}else{console.error("  REFUSING to use fallback - user specified:",urlCollectionName);// Try a more flexible match
...
_isFurnitureMode4=window.COLORFLEX_MODE==='FURNITURE'||window.location.pathname.includes('furniture');_isClothingMode3=window.COLORFLEX_MODE==='CLOTHING'||window.location.pathname.includes('clothing');if(_isFurnitureMode4||_isClothingMode3){_firstColorFlexCollection=appState.collections.find(function(c){var _c$patterns12;return(_c$patterns12=c.patterns)===null||_c$patterns12===void 0?void 0:_c$patterns12.some(function(p){return p.colorFlex===true;});});selectedCollection=_firstColorFlexCollection||appState.collections[0];console.log("  Last resort: Using ".concat((_selectedCollection2=selectedCollection)===null||_selectedCollection2===void 0?void 0:_selectedCollection2.name," (ColorFlex: ").concat(!!_firstColorFlexCollection,")"));}else{selectedCollection=appState.collections[0];}}}}console.log("  Selected collection source:",(_window$appState6=window.appState)!==null&&_window$appState6!==void 0&&_window$appState6.selectedCollection?"Shopify":"URL");console.log("  Final collection:",(_selectedCollection3=selectedCollection)===null||_selectedCollection3===void 0?void 0:_selectedCollection3.name);console.log("  Is fallback collection:",selectedCollection===appState.collections[0]?"YES":"NO");if(selectedCollection){_context32.n=25;break;}if(window.COLORFLEX_MODE==='BASSETT'){launchName=window.BASSETT_DEFAULT_COLLECTION||'hip-to-be-square';selectedCollection=appState.collections.find(function(c){return c&&typeof c.name==='string'&&c.name.trim().toLowerCase()===launchName.toLowerCase();});if(selectedCollection){console.log("Launching with default collection: "+selectedCollection.name);}}if(!selectedCollection){selectedCollection=appState.collections[0];if(selectedCollection)console.log("Using first collection as fallback: "+selectedCollection.name);}if(selectedCollection){_context32.n=25;break;}console.error("X No valid collection found.");return _context32.a(2);case 25:// ✅ Step 5: Set collection in appState
...
isVariantCollection=!appState.collections.some(function(c){return c.name===selectedCollectionName;});if(!(isVariantCollection&&appState.allCollections)){_context32.n=30;break;}// Try to find base collection using flexible matching
...
for(_i14=nameParts.length-1;_i14>0;_i14--){candidate=nameParts.slice(0,_i14).join('-');baseNameCandidates.push(candidate);}}// Find base collection using candidates
...
targetPatternName=urlPatternName||autoLoadPatternName;if(!targetPatternName){_context32.n=40;break;}console.log("🎯 Looking for target pattern:",targetPatternName,urlPatternName?"(from URL)":"(from auto-load)");// First try to find pattern in selected collection (with normalization)
...
if(window.COLORFLEX_MODE==='BASSETT'&&((_appState$curatedColo3=appState.curatedColors)!==null&&_appState$curatedColo3!==void 0&&_appState$curatedColo3.length||(_selectedCollection$c=selectedCollection.curatedColors)!==null&&_selectedCollection$c!==void 0&&_selectedCollection$c.length)){[400,1000,2000].forEach(function(delay){setTimeout(function(){var col=appState.selectedCollection||selectedCollection;var c=appState.curatedColors&&appState.curatedColors.length?appState.curatedColors:col&&col.curatedColors||[];if(c.length&&typeof populateCuratedColors==='function')populateCuratedColors(c);},delay);});}// Apply URL parameters for colors and scale after pattern loading
...
var isFurnitureMode=window.COLORFLEX_MODE==='FURNITURE'||appState.isInFurnitureMode;var isClothingMode=window.COLORFLEX_MODE==='CLOTHING'||window.COLORFLEX_SIMPLE_MODE===true&&window.location.pathname.includes('clothing');if(isFurnitureMode){updateFurniturePreview();}else if(appState.isInFabricMode||isClothingMode){renderFabricMockup();}else{updateRoomMockup();}};_applyColorsOnceReady();}catch(error){console.error('❌ Error applying URL colors:',error);}}// Apply pattern scale from URL
...
if((savedLayersParam||urlColorsRaw)&&sourceParam==='cart_saved_pattern'){console.log('🎨 Restoring saved colors from cart URL parameters...');console.log('  Source:',sourceParam);console.log('  Saved pattern ID:',savedPatternId);// Delay color restoration to ensure UI is ready; repeat to survive late UI resets.
[1000,2200,3800].forEach(function(delay){setTimeout(function(){restoreCartColors(savedLayersParam,urlColorsRaw);},delay);});}// Generic URL color fallback for non-cart sources:
...
if(appState.layerInputs[i]&&appState.layerInputs[i].circle){appState.layerInputs[i].circle.style.backgroundColor=lookupColor(colorToApply.color);}if(appState.layerInputs[i]){appState.layerInputs[i].color=colorToApply.color;}}// Force-sync appState.currentLayers from layerInputs by label (shadow-safe).
if(Array.isArray(appState.layerInputs)&&Array.isArray(appState.currentLayers)){appState.layerInputs.forEach(function(li){if(!li||!li.label||!li.input)return;var clIdx=appState.currentLayers.findIndex(function(layer){return layer&&layer.label===li.label;});if(clIdx!==-1)appState.currentLayers[clIdx].color=li.input.value;});}// Fire one update pass after state is synchronized.
setTimeout(function(){if(token!==window.__colorflexRestoreToken)return;console.log('🎨 Updating previews after cart color restoration');updatePreview();var isFurnitureMode=window.COLORFLEX_MODE==='FURNITURE'||appState.isInFurnitureMode;var isClothingMode=window.COLORFLEX_MODE==='CLOTHING'||window.COLORFLEX_SIMPLE_MODE===true&&window.location.pathname.includes('clothing');if(isFurnitureMode){updateFurniturePreview();}else if(appState.isInFabricMode||isClothingMode){renderFabricMockup();}else{updateRoomMockup();}},120);// Debug snapshot for reproducibility if needed.
window.__colorflexLastRestoreSnapshot={at:new Date().toISOString(),colorsApplied:normalizedColors.map(function(c){return c.color;}),layerInputs:(appState.layerInputs||[]).map(function(li){return{label:li.label,value:li.input&&li.input.value};}),currentLayers:(appState.currentLayers||[]).map(function(l){return{label:l.label,color:l.color,isShadow:l.isShadow===true};})};console.log('✅ Cart color restoration completed');};_applyWhenReady(0);}catch(error){console.error('❌ Error restoring cart colors:',error);}}// Ensure appState has a default
...
visualElement=document.createElement("img");visualElement.className="layer-thumbnail-img";var layerData=pattern.layers[layerImageIndex];var layerPath=typeof layerData==='string'?layerData:(layerData===null||layerData===void 0?void 0:layerData.path)||layerData;var layerUrl=normalizePath(layerPath);visualElement.src=layerUrl;console.log('[ColorFlex] Layer thumbnail URL:',layerUrl);visualElement.alt=layer.label;visualElement.onerror=function(){visualElement.style.background='#1a202c';};layerImageIndex++;// Only increment for actual image layers
}// Label
...
container.appendChild(visualElement);container.appendChild(labelEl);container.appendChild(input);thumbnailContainer.appendChild(container);});console.log("✅ Simple mode layer inputs created:",inputLayers.length,"layers");return;}// Show layer inputs container and heading for patterns with layers (ColorFlex only; standard patterns returned above)
...
var isStandardPattern=patternIsStandard(pattern,appState.selectedCollection);var colorLayersHeading=document.getElementById('colorLayersHeading');if(colorLayersHeading){colorLayersHeading.style.display=isStandardPattern?'none':'';console.log("\uD83D\uDCCB Color Layers heading: ".concat(isStandardPattern?'hidden':'shown'," for pattern type"));}// Show the color lock button for ColorFlex patterns
console.log("🔍 COLOR LOCK BUTTON CHECK (ColorFlex Pattern):");console.log("  Pattern:",pattern.name);console.log("  Has layers:",((_pattern$layers=pattern.layers)===null||_pattern$layers===void 0?void 0:_pattern$layers.length)||0);var colorLockBtn=document.getElementById('colorLockBtn');console.log("  Button found:",!!colorLockBtn);if(colorLockBtn){colorLockBtn.style.display='';console.log('✅ SHOWING COLOR LOCK BUTTON: ColorFlex pattern has',pattern.layers.length,'customizable layers');console.log('  Button display style:',colorLockBtn.style.display||'default (visible)');}else{console.warn('⚠️ Color lock button not found in DOM');}// ✅ Save color lock buffer BEFORE handlePatternSelection (if colors are locked).
...
addSaveButton();// First-time onboarding: arrow + tooltip to layer, then to curated colors (once per session)
if(appState.layerInputs&&appState.layerInputs.length>0){setTimeout(function(){runOnboardingSequence();},600);}}catch(e){console.error("❌ Error in populateLayerInputs:",e);}}if(USE_GUARD&&DEBUG_TRACE){populateLayerInputs=guard(traceWrapper(populateLayerInputs,"populateLayerInputs"));}else if(USE_GUARD){populateLayerInputs=guard(populateLayerInputs,"populateLayerInputs");}if(USE_GUARD&&DEBUG_TRACE){populateLayerInputs=guard(traceWrapper(populateLayerInputs,"populateLayerInputs"));}else if(USE_GUARD){populateLayerInputs=guard(populateLayerInputs,"populateLayerInputs");}// ============================================================================
...
handlePatternSelection(pattern.slug||pattern.id||pattern.name,appState.colorsLocked);// ✅ NEW ARCHITECTURE: Look up mockupLayers from variant collection if available
...
if(!(appState.colorsLocked&&(((_savedColorBuffer=savedColorBuffer)===null||_savedColorBuffer===void 0?void 0:_savedColorBuffer.length)>0||appState.colorLockFullBuffer&&appState.colorLockFullBuffer.length>0))){_context33.n=7;break;}bufferToRestore=appState.colorLockFullBuffer&&appState.colorLockFullBuffer.length>=appState.layerInputs.length?appState.colorLockFullBuffer:savedColorBuffer;if(bufferToRestore&&bufferToRestore.length>0){console.log('🔒 Color lock: Restoring colors from',bufferToRestore===appState.colorLockFullBuffer?'full buffer':'saved buffer','to',appState.layerInputs.length,'layers');appState.layerInputs.forEach(function(layer,index){var savedColor=index<bufferToRestore.length?bufferToRestore[index]:bufferToRestore[bufferToRestore.length-1];layer.input.value=savedColor;var hex=lookupColor(savedColor)||"#FFFFFF";layer.circle.style.backgroundColor=hex;var clIdx=appState.currentLayers.findIndex(function(l){return l.label===layer.label;});if(clIdx!==-1)appState.currentLayers[clIdx].color=savedColor;console.log("  Restored layer ".concat(index," (").concat(layer.label,"): ").concat(savedColor));});}// Trigger preview update with new colors (check mode first)
updatePreview();// ✅ Check mode and call appropriate render function
isFurnitureModeRestore=window.COLORFLEX_MODE==='FURNITURE'||appState.isInFurnitureMode;isClothingModeRestore=window.COLORFLEX_MODE==='CLOTHING'||window.COLORFLEX_SIMPLE_MODE===true&&window.COLORFLEX_MODE!=='FURNITURE';if(!isFurnitureModeRestore){_context33.n=4;break;}console.log('🔒 Color lock restore: Calling updateFurniturePreview() for furniture mode');if(!(typeof updateFurniturePreview==='function')){_context33.n=2;break;}_context33.n=1;return updateFurniturePreview();case 1:_context33.n=3;break;case 2:console.error('❌ updateFurniturePreview not available!');case 3:_context33.n=7;break;case 4:if(!(appState.isInFabricMode||isClothingModeRestore)){_context33.n=6;break;}console.log('🔒 Color lock restore: Calling renderFabricMockup() for clothing/fabric mode');_context33.n=5;return renderFabricMockup();case 5:_context33.n=7;break;case 6:console.log('🔒 Color lock restore: Calling updateRoomMockup() for wallpaper mode');if(window.COLORFLEX_MODE!=='BASSETT')updateRoomMockup();case 7:// ✅ Restore saved scale (BEFORE final preview updates)
...
console.log("  🎨 Processing pattern layers:");for(var i=0;i<patternLayers.length;i++){var layer=patternLayers[i];// JSON "isShadow": true is the source of truth for compositing (no UI input, fixed multiply/opacity). Path-based is fallback only.
var pathStr=layer&&(layer.path||layer.proofPath)?layer.path||layer.proofPath:'';var isTrueShadow=layer.isShadow===true||pathStr&&(pathStr.toUpperCase().includes('_SHADOW_')||pathStr.toUpperCase().includes('SHADOW_LAYER')||pathStr.toUpperCase().includes('ISSHADOW'));if(!isTrueShadow){// Use label for THIS pattern layer index i so shadow layers don't steal the next layer's name
var originalLabel=layerLabels[i]||"Pattern Layer ".concat(i+1);var layerObj={label:originalLabel,color:designerColors[colorIndex++]||"Snowbound",path:layer.path||"",isBackground:false,isShadow:false,isWallPanel:false,tintWhite:tintWhite,inputId:"layer-".concat(inputIndex++),patternLayerIndex:i};allLayers.push(layerObj);console.log("    \u2705 Added pattern layer: \"".concat(originalLabel,"\" (designer color ").concat(colorIndex-1,")"));}else{// Shadow layers: no UI input, fixed in compositing (e.g. Iron Ore). Skip their slot in designer_colors
// so the next layer gets the correct color (e.g. Flower gets PeriStyle Brass, not Iron Ore).
colorIndex++;var _layerObj={label:"Shadow ".concat(i+1),color:null,path:layer.path||"",isBackground:false,isShadow:true,isWallPanel:false,tintWhite:tintWhite,inputId:null,patternLayerIndex:i};allLayers.push(_layerObj);console.log("    \u2705 Added shadow layer: \"Shadow ".concat(i+1,"\" (skipped color slot)"));}}// ✅ ADD EXTRAS/PILLOWS INPUT AT THE END (furniture collections only)
...
isStandardBg=patternIsStandard(appState.currentPattern,appState.selectedCollection);if(isStandardBg){backgroundColor="#000";// Black background for standard patterns
...
if(!(patternToRender.tintWhite&&patternToRender.baseComposite)){_context0.n=8;break;}console.log("🎨 Rendering tint white pattern");baseImage=new Image();baseImage.crossOrigin="Anonymous";baseImage.src=normalizePath(patternToRender.baseComposite);_context0.n=7;return new Promise(function(resolve,reject){baseImage.onload=function(){var _appState$currentLaye6;var scaleMultiplier=appState.scaleMultiplier||.5;var imgAspect=baseImage.width/baseImage.height;var maxSize=canvasSize*scaleMultiplier;var drawWidth,drawHeight,offsetX,offsetY;if(imgAspect>1){drawWidth=Math.min(maxSize,canvasSize);drawHeight=drawWidth/imgAspect;}else{drawHeight=Math.min(maxSize,canvasSize);drawWidth=drawHeight*imgAspect;}offsetX=(canvasSize-drawWidth)/2;offsetY=(canvasSize-drawHeight)/2;previewCtx.fillStyle=backgroundColor;previewCtx.fillRect(offsetX,offsetY,drawWidth,drawHeight);previewCtx.drawImage(baseImage,offsetX,offsetY,drawWidth,drawHeight);// Apply tint to white areas
var imageData;try{imageData=previewCtx.getImageData(offsetX,offsetY,drawWidth,drawHeight);}catch(e){console.warn("⚠️ Canvas tainted, skipping preview tinting:",e.message);resolve();return;}var data=imageData.data;var wallColor=lookupColor(((_appState$currentLaye6=appState.currentLayers[0])===null||_appState$currentLaye6===void 0?void 0:_appState$currentLaye6.color)||"Snowbound");var hex=wallColor.replace("#","");var rTint=parseInt(hex.substring(0,2),16);var gTint=parseInt(hex.substring(2,4),16);var bTint=parseInt(hex.substring(4,6),16);for(var i=0;i<data.length;i+=4){var r=data[i],g=data[i+1],b=data[i+2];if(r>240&&g>240&&b>240){data[i]=rTint;data[i+1]=gTint;data[i+2]=bTint;}}previewCtx.putImageData(imageData,offsetX,offsetY);resolve();};baseImage.onerror=reject;});case 7:_context0.n=9;break;case 8:if(!((_patternToRender$laye2=patternToRender.layers)!==null&&_patternToRender$laye2!==void 0&&_patternToRender$laye2.length)){_context0.n=9;break;}isLayerShadow=function isLayerShadow(l){if(!l)return true;var path=typeof l==='string'?l:l.path||l.proofPath||'';return l&&_typeof(l)==='object'&&l.isShadow===true||path&&(String(path).toUpperCase().includes('_SHADOW_')||String(path).toUpperCase().includes('SHADOW_LAYER')||String(path).toUpperCase().includes('ISSHADOW'));};firstLayer=patternToRender.layers.find(function(l){return!isLayerShadow(l);});if(!firstLayer){_context0.n=9;break;}tempImg=new Image();tempImg.crossOrigin="Anonymous";// Handle both string layers (botanicals) and object layers (other patterns)
firstLayerPath=typeof firstLayer==='string'?firstLayer:firstLayer.path||firstLayer;tempImg.src=normalizePath(firstLayerPath);_context0.n=9;return new Promise(function(resolve){tempImg.onload=function(){// ✅ FIX: Use declared pattern size for aspect ratio, not image dimensions
...
if(isHalfDropTiling(pattern===null||pattern===void 0?void 0:pattern.tilingType)){repeatStr+='HD';}else if(normalizeTilingType(pattern===null||pattern===void 0?void 0:pattern.tilingType)==='straight'){repeatStr+='S';}}if(repeatStr){return"<br><span style=\"font-size: 0.85em; color: rgba(255, 255, 255, 0.6); font-weight: normal;\">Pattern Repeat: ".concat(repeatStr,"</span>");}return'';}function normalizeTilingType(value){return String(value||'').trim().toLowerCase().replace(/[_\s]+/g,'-');}function isHalfDropTiling(value){var t=normalizeTilingType(value);return t==='half-drop'||t==='halfdrop'||t==='hd';}/**
...
effectiveMockup=getEffectiveRoomMockup(appState.selectedCollection);mockupWidthInches=effectiveMockup.mockupWidthInches||90;mockupHeightInches=effectiveMockup.mockupHeightInches||60;// Force the room mockup preview box to a stable 4:3 aspect (width:height),
// so the room scene fills the box without letterbox bars.
mockupAspect=3/4;rect=dom.roomMockup.getBoundingClientRect();cssW=Math.max(1,Math.round(rect.width||dom.roomMockup.clientWidth||dom.roomMockup.offsetWidth||600));cssH=Math.max(1,Math.round(cssW*mockupAspect));dpr=window.devicePixelRatio||1;snap=function snap(v){return Math.round(v*dpr)/dpr;};// align to device grid
mod=function mod(a,b){return(a%b+b)%b;};// Calculate px per inch based on actual mockup dimensions (collection or user override)
...
if(dom.roomMockup){_context14.n=2;break;}console.error("❌ roomMockup element not found in DOM");return _context14.a(2);case 2:if(appState.currentPattern){_context14.n=3;break;}console.error("❌ No current pattern selected");return _context14.a(2);case 3:if(furnitureConfig){_context14.n=4;break;}console.log("🔄 Loading furniture config...");_context14.n=4;return loadFurnitureConfig();case 4:if(furnitureConfig){_context14.n=5;break;}console.error("❌ furnitureConfig still not loaded after attempt");return _context14.a(2);case 5:// ===== MULTI-SCALE FURNITURE: Only for furniture-simple page =====
...
testPaths=[{name:"mockup",path:furniture.mockup},{name:"wallMask",path:furniture.wallMask},{name:"base",path:furniture.base},{name:"extras",path:furniture.extras}];console.log("🔍 TESTING FILE EXISTENCE:");testPaths.forEach(function(_ref22){var name=_ref22.name,path=_ref22.path;if(path){var testImg=new Image();testImg.onload=function(){return console.log("\u2705 ".concat(name," file exists: ").concat(path));};testImg.onerror=function(){return console.log("\u274C ".concat(name," file MISSING: ").concat(path));};testImg.src=normalizePath(path);}else{console.log("\u26A0\uFE0F ".concat(name," path not defined in config"));}});// Get layer mapping for furniture collection
...
_ctx3.fillStyle="transparent";_ctx3.fillRect(0,0,canvas.width,canvas.height);console.log("🧹 Canvas cleared with white background");_ctx3.clearRect(0,0,canvas.width,canvas.height);_ctx3.fillStyle="#F5F5F5";_ctx3.fillRect(0,0,canvas.width,canvas.height);// ❌ REMOVED: The problematic settings update that was resetting zoom
// NO LONGER UPDATING furnitureViewSettings here - using preserved settings
console.log("🔍 FURNITURE VIEW SETTINGS:");console.log("  Scale:",furnitureViewSettings.scale);console.log("  Offset X:",furnitureViewSettings.offsetX);console.log("  Offset Y:",furnitureViewSettings.offsetY);_context14.p=7;console.log("🏗️ =========================");console.log("🏗️ FURNITURE RENDERING SEQUENCE (WITH WALL MASK)");console.log("🏗️ =========================");// ✅ CRITICAL: Set flag to prevent other render functions from interfering
...
if(shouldReloadBackgroundImage){console.log("1\uFE0F\u20E3 Drawing mockup base (room scene) - furniture type changed from ".concat(oldFurnitureType||'none'," to ").concat(currentFurnitureType,", reloading background image"));}else{console.log("1\uFE0F\u20E3 Drawing mockup base (room scene) - furniture type unchanged (".concat(currentFurnitureType,"), redrawing existing background"));}mockupPath=furniture.mockup;if(!mockupPath){_context14.n=9;break;}console.log("  Mockup path:",mockupPath);// ✅ FIX: Use same zoom state as other furniture layers for consistent scaling
_context14.n=8;return drawFurnitureLayer(_ctx3,mockupPath,{zoomState:frozenZoomState})["catch"](function(err){console.error("❌ Failed to load mockup:",err);_ctx3.fillStyle="#E5E7EB";_ctx3.fillRect(0,0,canvas.width,canvas.height);console.log("🔄 Drew fallback background due to mockup failure");});case 8:console.log("✅ Room mockup base drawn");_context14.n=10;break;case 9:console.error("❌ No mockup path in furniture config");_ctx3.fillStyle="#E5E7EB";_ctx3.fillRect(0,0,canvas.width,canvas.height);case 10:// ===== STEP 2: Draw sofa base =====
...
});case 15:console.log("  ✅ Sofa base shadow completed");_context14.n=17;break;case 16:console.error("❌ No base path in furniture config");case 17:// ✅ ADD DELAY TO ENSURE SOFA BASE IS FULLY RENDERED
console.log("⏳ Waiting for sofa base to complete before patterns...");_context14.n=18;return new Promise(function(resolve){return setTimeout(resolve,50);});case 18:// ===== STEP 3: Draw pattern layers =====
...
furnitureLayerPath=normalizePath("data/collections/".concat(_collectionBaseName2,"-fur/layers/").concat(_furnitureType2,"/").concat(finalFileName));console.log("    \uD83D\uDD04 Converted path (new format): ".concat(layer.path," \u2192 ").concat(furnitureLayerPath));}// ✅ CRITICAL: Normalize path before passing to drawFurnitureLayer
...
zoomState:frozenZoomState});case 50:console.log("✅ Wall color applied via mask as final composite layer");_context14.n=52;break;case 51:console.warn("⚠️ No wallMask path in furniture config");case 52:console.log("🎉 =========================");console.log("🎉 FURNITURE RENDERING COMPLETE (WITH WALL MASK)");console.log("🎉 =========================");// ===== STEP 6: Display result =====
...
function _startApp(){_startApp=_asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee33(){return _regenerator().w(function(_context41){while(1)switch(_context41.p=_context41.n){case 0:if(!appInitializing){_context41.n=1;break;}console.warn("⚠️ App initialization already in progress, skipping duplicate call");return _context41.a(2);case 1:if(!isAppReady){_context41.n=2;break;}console.warn("⚠️ App already initialized, skipping duplicate call");return _context41.a(2);case 2:appInitializing=true;_context41.p=3;_context41.n=4;return initializeApp();case 4:_context41.n=5;return loadFurnitureConfig();case 5:isAppReady=true;console.log("✅ App fully initialized and ready.");// Add chameleon icon next to save button
...
// ============================================================================
function getPatternType(pattern,collection){var _collection$elements;if((collection===null||collection===void 0?void 0:collection.name)==="wall-panels")return"wall-panel";if(pattern!==null&&pattern!==void 0&&pattern.tintWhite)return"tint-white";if(collection!==null&&collection!==void 0&&(_collection$elements=collection.elements)!==null&&_collection$elements!==void 0&&_collection$elements.length)return"element-coloring";// Only treat as ColorFlex when explicitly flagged; patterns with layers but no flag are standard
if((pattern===null||pattern===void 0?void 0:pattern.colorFlex)===true&&pattern!==null&&pattern!==void 0&&pattern.layers&&pattern.layers.length>0)return"colorflex";return"standard";}function getColorMapping(patternType,currentLayers,layerIndex){switch(patternType){case"wall-panel":return currentLayers[layerIndex+2];// Skip wall + background
case"standard":case"colorflex":var inputLayers=currentLayers.filter(function(layer){return!layer.isShadow;});return inputLayers[layerIndex+1];// Skip background
case"element-coloring":var inputLayersElement=currentLayers.filter(function(layer){return!layer.isShadow;});return inputLayersElement[layerIndex+1];default:return currentLayers[layerIndex+1];}}// Add fabric tuning controls
...
isMultiScaleClothing=(hasNestedMockupLayers||pattern._originalMockupLayers)&&isClothingMode;if(!isMultiScaleClothing){_context43.n=8;break;}console.log("👗 MULTI-SCALE CLOTHING: Detected nested mockupLayers format");// ✅ ENFORCE SCALE 1.0 for standard clothing page (not simple mode)
...
selectedGarment=appState.selectedGarment;console.log("\uD83D\uDC54 Selected garment: ".concat(selectedGarment));garmentLayers=pattern._originalMockupLayers[selectedGarment];if(garmentLayers){_context43.n=6;break;}console.error("\u274C No mockupLayers found for garment: ".concat(selectedGarment));console.log("Available garments:",Object.keys(pattern._originalMockupLayers));return _context43.a(2);case 6:scaleLayers=garmentLayers[appState.selectedClothingScale];if(!(!scaleLayers||scaleLayers.length===0)){_context43.n=7;break;}console.error("\u274C No layers found for scale: ".concat(appState.selectedClothingScale,"X"));console.log("Available scales for ".concat(selectedGarment,":"),Object.keys(garmentLayers));return _context43.a(2);case 7:console.log("\uD83D\uDC57 Using ".concat(scaleLayers.length," layer(s) for ").concat(selectedGarment," @ ").concat(appState.selectedClothingScale,"X scale"));// Flatten the nested structure: Replace pattern.mockupLayers with the scale-specific array
...
if(!fabricConfig.base){_context43.n=12;break;}fabricBase=new Image();fabricBase.crossOrigin="anonymous";_context43.n=11;return new Promise(function(resolve,reject){fabricBase.onload=resolve;fabricBase.onerror=reject;fabricBase.src=normalizePath(fabricConfig.base.startsWith('data/')?fabricConfig.base:'data/'+fabricConfig.base);});case 11:fabricBaseWidth=fabricBase.naturalWidth||fabricBase.width;fabricBaseHeight=fabricBase.naturalHeight||fabricBase.height;console.log("\uD83D\uDCD0 Fabric base loaded at NATIVE resolution: ".concat(fabricBaseWidth,"x").concat(fabricBaseHeight));// For clothing mode: Create tinted dress-base canvas at NATIVE 4K resolution
...
if(!isClothingMode){_context43.n=21;break;}_context43.p=18;// Gloss layers are in data/mockups/clothing/
...
if(!(roomMockup.tagName==='IMG')){_context43.n=28;break;}roomMockup.src=dataURL;console.log("✅ Set fabric mockup as img src");_context43.n=30;break;case 28:if(!isClothingMode){_context43.n=29;break;}// CLOTHING MODE: Append actual canvas element (no background-image!)
...
var roomMockup=document.getElementById('roomMockup');if(roomMockup){roomMockup.src=canvas.toDataURL();console.log("✅ Simple fabric test complete");}};img.src=normalizePath("data/fabric/fabric-base.png");};// Enhanced color parsing function for proof generation
function parseColorEnhanced(_x17){return _parseColorEnhanced.apply(this,arguments);}// ============================================================================
...
var backgroundColor=lookupColor(colorArray[0]||"Snowbound");_ctx5.fillStyle=backgroundColor;_ctx5.fillRect(0,0,canvas.width,canvas.height);_ctx5.drawImage(baseImage,0,0,canvas.width,canvas.height);resolve();};baseImage.onerror=reject;});case 5:_context47.n=10;break;case 6:if(!((_targetPattern$layers2=targetPattern.layers)!==null&&_targetPattern$layers2!==void 0&&_targetPattern$layers2.length)){_context47.n=10;break;}console.log("🎨 Rendering layered pattern for proof");// Path-based shadow detection (match updatePreview/print): JSON isShadow or path contains _SHADOW_ etc.
layerIsShadow=function layerIsShadow(l){if(l&&l.isShadow===true)return true;var pathStr=l&&(l.path||l.proofPath)?String(l.path||l.proofPath):'';return pathStr&&(pathStr.toUpperCase().includes('_SHADOW_')||pathStr.toUpperCase().includes('SHADOW_LAYER')||pathStr.toUpperCase().includes('ISSHADOW'));};firstLayer=targetPattern.layers.find(function(l){return!layerIsShadow(l);});if(!firstLayer){_context47.n=10;break;}tempImg=new Image();tempImg.crossOrigin="Anonymous";// ⚠️ CRITICAL: Use proofPath (high-res ~3600px) NOT path (preview ~1400px)
// proofPath: ./data/collections/{collection}/proof-layers/*.jpg
// path: ./data/collections/{collection}/layers/*.jpg
tempImg.src=normalizePath(firstLayer.proofPath||firstLayer.path);_context47.n=7;return new Promise(function(resolve){tempImg.onload=function(){// ⚠️ CRITICAL: Canvas size represents 24" wide proof, NOT the layer image size
...
resolve({offsetX:0,offsetY:0,patternDisplayWidth:canvas.width,patternDisplayHeight:canvas.height,scaleMultiplier:effectiveScale});};tempImg.onerror=function(){return resolve(null);};});case 7:patternBounds=_context47.v;if(!patternBounds){_context47.n=10;break;}// colorArray = [background, ...pattern non-shadow colors]. We already used colorArray[0] for canvas fill.
// Pattern layers (shadow, bamboo, outline): only non-shadow get a color, from colorArray[1], colorArray[2], ...
nonShadowColorIndex=1;// Render each layer using the exact same logic as updatePreview
_loop11=/*#__PURE__*/_regenerator().m(function _loop11(layerIndex){var layer,isShadow,layerColor;return _regenerator().w(function(_context46){while(1)switch(_context46.n){case 0:layer=targetPattern.layers[layerIndex];isShadow=layerIsShadow(layer);// Safety fallback for patterns where shadow metadata is missing:
...
if(!isShadow&&nonShadowColorIndex>=colorArray.length){console.warn("\u26A0\uFE0F Proof fallback: treating layer ".concat(layerIndex," as shadow (no mapped non-shadow color left)"));isShadow=true;}layerColor=!isShadow?lookupColor(colorArray[nonShadowColorIndex++]||"Snowbound"):null;console.log("\uD83D\uDD27 Proof layer ".concat(layerIndex," with color:"),layerColor,'using',layer.proofPath?'PROOF PATH (high-res)':'preview path (fallback)');_context46.n=1;return new Promise(function(resolve){// Simplified proof generation - just composite the layers at full size
...
var tilingType=targetPattern.tilingType||"";var isHalfDrop=isHalfDropTiling(tilingType);console.log("\uD83D\uDD27 Proof tiling: ".concat(isHalfDrop?'HALF-DROP':'NORMAL'," (tilingType: \"").concat(tilingType,"\")"));// Tile the pattern across the canvas at the scaled size
...
var modified=false;for(var _i1=0;_i1<bytes.length-20;_i1++){if(bytes[_i1]===0xFF&&bytes[_i1+1]===0xE0){// Check if this is JFIF marker
if(bytes[_i1+4]===0x4A&&bytes[_i1+5]===0x46&&bytes[_i1+6]===0x49&&bytes[_i1+7]===0x46){// Found JFIF header
// Set density unit to 1 (dots per inch)
bytes[_i1+11]=0x01;// Set X density (DPI) - 2 bytes, big-endian
bytes[_i1+12]=calculatedDPI>>8&0xFF;bytes[_i1+13]=calculatedDPI&0xFF;// Set Y density (DPI) - 2 bytes, big-endian
bytes[_i1+14]=calculatedDPI>>8&0xFF;bytes[_i1+15]=calculatedDPI&0xFF;console.log("\u2705 DPI metadata set to ".concat(calculatedDPI," DPI (ensures 24\" width in image editors)"));modified=true;break;}}}if(!modified){console.warn('⚠️ Could not find JFIF header to set DPI metadata');}// Create blob from modified bytes
 ...
 */function generatePatternProofDataUrl(_x34,_x35,_x36,_x37,_x38){return _generatePatternProofDataUrl.apply(this,arguments);}function _generatePatternProofDataUrl(){_generatePatternProofDataUrl=_asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee40(patternName,collectionName,colorArray,quality,targetSize){var proofCanvas,outputSize,out,ctx,srcW,srcH,scale,drawW,drawH,dx,dy,jpegQuality;return _regenerator().w(function(_context50){while(1)switch(_context50.n){case 0:_context50.n=1;return generatePatternProof(patternName,collectionName,colorArray,appState&&appState.currentScale?appState.currentScale:null);case 1:proofCanvas=_context50.v;outputSize=Number(targetSize)>0?Number(targetSize):3600;out=document.createElement('canvas');out.width=outputSize;out.height=outputSize;ctx=out.getContext('2d');// Cover-fit onto fixed 3600x3600 canvas so output is always full-size.
srcW=proofCanvas.width||1;srcH=proofCanvas.height||1;scale=Math.max(outputSize/srcW,outputSize/srcH);drawW=srcW*scale;drawH=srcH*scale;dx=(outputSize-drawW)/2;dy=(outputSize-drawH)/2;ctx.drawImage(proofCanvas,dx,dy,drawW,drawH);jpegQuality=typeof quality==='number'?quality:0.95;return _context50.a(2,out.toDataURL('image/jpeg',jpegQuality));}},_callee40);}));return _generatePatternProofDataUrl.apply(this,arguments);}window.generatePatternProofDataUrl=generatePatternProofDataUrl;function generatePrintPreviewDataUrl(){return _generatePrintPreviewDataUrl.apply(this,arguments);}function _generatePrintPreviewDataUrl(){_generatePrintPreviewDataUrl=_asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee41(){var result;return _regenerator().w(function(_context51){while(1)switch(_context51.n){case 0:_context51.n=1;return generatePrintPreview({silent:true});case 1:result=_context51.v;return _context51.a(2,result&&typeof result.dataUrl==='string'?result.dataUrl:null);}},_callee41);}));return _generatePrintPreviewDataUrl.apply(this,arguments);}window.generatePrintPreviewDataUrl=generatePrintPreviewDataUrl;/**
 ...
 */function setFurnitureScale(_x40){return _setFurnitureScale.apply(this,arguments);}/**
 * Set furniture type and re-render mockup
 * @param {string} furnitureType - Furniture type ("Sofa-Capitol", "Sofa-Kite")
 */function _setFurnitureScale(){_setFurnitureScale=_asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee43(scale){var normalizedScale,scaleDisplay,scaleButtons;return _regenerator().w(function(_context53){while(1)switch(_context53.n){case 0:console.log("\uD83E\uDE91 Setting furniture scale to ".concat(scale,"X"));// ✅ NORMALIZE SCALE: Remove "X" suffix if present to match JSON key format
...
if(!(typeof updateFurniturePreview==='function')){_context53.n=2;break;}console.log("  \uD83D\uDD04 Re-rendering furniture preview at ".concat(scale,"X"));_context53.n=1;return updateFurniturePreview();case 1:_context53.n=3;break;case 2:console.warn('⚠️ updateFurniturePreview not available');case 3:return _context53.a(2);}},_callee43);}));return _setFurnitureScale.apply(this,arguments);}function setFurnitureType(_x41,_x42){return _setFurnitureType.apply(this,arguments);}// Expose furniture functions globally with event handling
function _setFurnitureType(){_setFurnitureType=_asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee44(furnitureType,event){var furnitureButtons;return _regenerator().w(function(_context54){while(1)switch(_context54.n){case 0:// Prevent event propagation if event is provided
...
if(!(typeof updateFurniturePreview==='function')){_context54.n=2;break;}console.log("  \uD83D\uDD04 Re-rendering furniture preview with ".concat(furnitureType));_context54.n=1;return updateFurniturePreview();case 1:_context54.n=3;break;case 2:console.warn('⚠️ updateFurniturePreview not available');case 3:return _context54.a(2);}},_callee44);}));return _setFurnitureType.apply(this,arguments);}window.setFurnitureScale=setFurnitureScale;window.setFurnitureType=function(furnitureType,event){return setFurnitureType(furnitureType,event);};// Helper functions for UI increment/decrement
...
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _bassett_layer_stack_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./bassett-layer-stack.js */ "./src/bassett-layer-stack.js");
/**
...
if (typeof window !== 'undefined') {
  ...
  window.BASSETT_LAYER_STACK = _bassett_layer_stack_js__WEBPACK_IMPORTED_MODULE_0__.BASSETT_LAYER_STACK;
  var dispFiles = _bassett_layer_stack_js__WEBPACK_IMPORTED_MODULE_0__.BASSETT_LAYER_STACK.map(function (l) {
    return l && (l.displacementFile || l.file);
  }).filter(Boolean);
  console.log('[Bassett] layer stack set from bassett-layer-stack.js, layers:', _bassett_layer_stack_js__WEBPACK_IMPORTED_MODULE_0__.BASSETT_LAYER_STACK.length, 'files:', dispFiles.join(', '));
...
/* harmony export */   BASSETT_LAYER_STACK: () => (/* binding */ BASSETT_LAYER_STACK),
/* harmony export */   getLayerUrl: () => (/* binding */ getLayerUrl),
/* harmony export */   getLayersBaseUrl: () => (/* binding */ getLayersBaseUrl)
/* harmony export */ });
/**
...
var BASSETT_LAYER_STACK = [{
  ...
  id: 'sofa-displaced-1',
  displacementFile: 'sofa_disp1.png',
  type: 'pattern-displaced',
  colorFlexIndex: null
}, {
  id: 'sofa-displaced-2',
  displacementFile: 'sofa_disp2.png',
  type: 'pattern-displaced',
  colorFlexIndex: null,
  transform: {
    scale: .94,
  ...
  id: 'pillow1-displaced',
  displacementFile: 'pillow1_disp.png',
  type: 'pattern-displaced',
  colorFlexIndex: null,
  transform: {
    scale: .95,
  ...
  id: 'pillow2-displaced',
  displacementFile: 'pillow2_disp.png',
  type: 'pattern-displaced',
  colorFlexIndex: null,
  transform: {
    scale: .95,
  ...
  id: 'pillow3-displaced',
  displacementFile: 'pillow3_disp.png',
  type: 'pattern-displaced',
  colorFlexIndex: null,
  transform: {
    scale: .95,
...
function getLayerUrl(layer, baseUrl) {
  var base = baseUrl || getLayersBaseUrl();
  var file = layer.displacementFile || layer.file;
...
(() => {
/*!******************************!*\
  !*** ./src/index.bassett.js ***!
  \******************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _bassett_flags_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./bassett-flags.js */ "./src/bassett-flags.js");
...
//# sourceMappingURL=color-flex-bassett.js.map
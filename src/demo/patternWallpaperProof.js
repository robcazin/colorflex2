...
import { ENGINE_STYLE_IDS } from '../color/paletteStyles.js';
...
/**
 * @param {import('./patternWallpaperProofConfig.js').PATTERN_WALLPAPER_PROOF_MANIFEST[0]} def
 */
...
export async function runPatternWallpaperProof(mount) {
  ...
  /** @type {Map<string, { mode: string, images: HTMLImageElement[] }>} */
  ...
  try {
    ...
    for (const { def, assets, error } of loads) {
      if (assets) assetsById.set(def.id, assets);
    }
  ...
  function syncPaletteAndPaint() {
    ...
    rows.forEach((row) => {
      if (typeof row._paintRow === 'function') {
        row._paintRow(locked);
      }
    });
  }

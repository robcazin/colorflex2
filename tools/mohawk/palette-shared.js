/**
 * Shared color / image helpers for Mohawk palette CLI experiments.
 * Used by extract-palette-atmospheric.js and compare-atmospheric.js.
 * Does not modify the live app or palette bridge.
 */
...
const DISCLAIMER =
  'Image-derived test palette only — not manufacturing color accuracy.';
...
function chromaFromAB(a, b) {
  return Math.sqrt(a * a + b * b);
}

function hueDegFromAB(a, b) {
...
function downloadUrl(urlString) {
  return new Promise((resolve, reject) => {
    const lib = new URL(urlString).protocol === 'https:' ? https : http;
    const req = lib.get(
      urlString,
...
async function loadInputBuffer(input) {
  ...
  if (isRemote(s)) return downloadUrl(s);
...
function averagePaletteSaturation(palette) {
  ...
  return Math.round((t / palette.length) * 1000) / 1000;
}

function slugifyInput(input) {
  const base = path.basename(String(input).replace(/\?.*$/, ''), path.extname(String(input)));
  return base.replace(/[^a-zA-Z0-9._-]+/g, '-').replace(/^-+|-+$/g, '') || 'carpet';
}

/**
 * Weighted k-means in Lab space.
 */
function weightedLabKMeans(labL, labA, labB, weight, indexList, k, maxIters) {
...
async function loadMacroRgbSample(input, opts) {
  const blurSigma = opts && opts.blurSigma != null ? opts.blurSigma : 2.5;
  const maxWidth = opts && opts.maxWidth != null ? opts.maxWidth : 140;
  const subsampleTarget = opts && opts.subsampleTarget != null ? opts.subsampleTarget : 9000;
  ...
  if (nPix < 80) throw new Error(`Too few opaque pixels (${nPix}) after macro preprocess.`);
  const sample = subsampleDeterministic(opaque, Math.min(subsampleTarget, nPix));
  return { sample, info, nPix, buf };
}
